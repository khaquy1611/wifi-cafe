    ...
    return (
        <section className="container home-page-product">
            ...
            <Drawer
                ...
                bodyStyle={{ padding: 0 }}
            >
                <Booking qrCode={qrCode} onHide={onCloseDrawer} socket={socket} clientStoreInfo={clientStoreInfo} />
            </Drawer>
        </section>
    );
};
...
export const getServerSideProps = wrapper.getServerSideProps(async (context) => {
    ...
    const { qrCode } = parseCookies(context);

    const q = (query.q || qrCode) as string;
    ...
    await store.dispatch(getClientStoreInfo(q));
    await store.dispatch(getClientCategory(q));
    await store.dispatch(getClientProducts(q));
