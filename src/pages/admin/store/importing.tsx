...
const importing = () => {
    ...
    const [dataImport, setDataImport] = useState<dataWareHouseRes>();
                    ...
                    <Space>
                        <Button
                            ...
                            onClick={() => {
                                ...
                                setTypeBtnVisible(record.status !== 'order' ? 'detail' : 'update');
    ...
    const onCancelReceipt = async (record: dataWareHouse) => {
        try {
            ...
            const res = await getListWareHouse({ ...query, group_id, store_id, type: 'import' });
            setDataImport(res);
    ...
    useEffect(() => {
        const fetchListImportReceipt = async () => {
            const res = await getListReceipt({ group_id, store_id, type: 'import' });
            setDataReceipt(res.data);
        };
        fetchListImportReceipt();
    }, []);
