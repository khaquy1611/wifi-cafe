...
if [ $? != 0 ]; then
   ...
   helm install --tiller-namespace $1 --namespace=$1 -n $HELM_RELEASE $URL_HELM_CHART -f server_config/${CI_ENVIRONMENT_NAME}.values.yaml --wait
else
   ...
   helm upgrade --tiller-namespace $1 --namespace=$1 $HELM_RELEASE $URL_HELM_CHART -f server_config/${CI_ENVIRONMENT_NAME}.values.yaml --wait
fi
